...
def detect_image(net, meta, custom_image_bgr, thresh=.5, hier_thresh=.5, nms=.45, debug=False):
    ...
    custom_image = cv.resize(custom_image,(lib.network_width(net), lib.network_height(net)), interpolation = cv.INTER_LINEAR)
    # import scipy.misc
...
if __name__ == "__main__":
    ...
    for i in di[843:]:
        ...
        stime = time.time()
        a = performDetect(cv.imread(f"images/{asdf}/{i}.png"), configPath="./cfg/yolov44.cfg",
                          weightPath="backup\\yolov4-4cls_last.weights", metaPath=f"./data/{asdf}.data",thresh=0.2,showImage=True)
        for b in a:
            print(b[0:2], end='')
