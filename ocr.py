...
def goto1(x, y, z=3):
    key.ra()
    while True:
        resul = fi(cv.imread("dataimg\\i.png"), 12, 202, 87, 133, pixelsearch=True).pixelmatch(getimgfromhwnd(hwnd))
        if resul[1][0] - x >= 40:
            key.p(leftk)
            key(altk)
            key(altk)
...
while True:
    while True:
        resul = fi(getimgfromhwnd(hwnd)[678:683, 1294:1312], 1265, 1285, 677, 684, True)
        resul.pixli.append([[241, 224, 224], [241, 230, 230]])
        if resul.match(getimgfromhwnd(hwnd))[0] > 0.99:
            break
        goto1(68, 28)
        while True:
            key(32, 600, 700)
            ...
            if not fili[3].pixpix(getimgfromhwnd(hwnd)):
                sleep(2)
                for i in pytesseract.image_to_string(getimgfromhwnd(hwnd)[388:418, 450:560]):
                    try:
                        int(i)
                    except:
                        break
            ...
            while time.time() - st < 90:
                resul = fili[2].pixpix(getimgfromhwnd(hwnd))
                if resul:
                    atf = True
                    break
                else:
    ...
    if fi(cv.imread('dataimg\\ddd.png'), 630, 722, 239, 310).match(getimgfromhwnd(hwnd))[0] > 0.99:
        break
    mo(613, 402)
    mo.c(500, 550)
    key(returnk, 500, 550)
    ...
    mo(135 + resul.pixpix(getimgfromhwnd(hwnd))[1], 262 + resul.pixpix(getimgfromhwnd(hwnd))[2])
    mo.c(500, 550)
